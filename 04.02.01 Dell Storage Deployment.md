...
- Dell’s CSI PowerMax driver is installed via the **Dell CSM Operator** (not via Helm) in newer releases.  
...
- **Unisphere REST endpoint version requirement**: Starting from some v2.x versions, only **Unisphere 10.0 REST endpoints** are supported. Ensure your storage array’s Unisphere is compatible.  

---

# 🛠 Steps to Deploy CSI PowerMax v2.14.0 in OpenShift

Here’s a rough step-by-step flow. You may need to adapt to your OpenShift version and environment.

| Step | What You Do |
|------|-------------|
| **1** | **Ensure Prerequisites:** <br>• Nodes must support the chosen protocol (FC, iSCSI, NVMe/TCP) <br>• Install multipath / device-mapper-multipath packages if required <br>• For iSCSI: ensure `iscsid` service is enabled on nodes (via MachineConfig in OpenShift) <br>• Ensure connectivity from nodes to PowerMax (zoning, IPs, routing) <br>• TLS certificate / secret for reverse proxy (if used) <br>• Prepare credentials for Unisphere (username/password) <br>• Ensure that the CSI driver images (on quay.io or allowed registry) are accessible |
...
| **3** | **Create Namespace & Secrets** <br>• Create a namespace (like `powermax`) <br>• Create a Secret `powermax-creds` containing base64-encoded username and password for the Unisphere API <br>• If TLS for reverse proxy is required, create `csirevproxy-tls-secret` in that namespace containing `tls.crt` and `tls.key` |
| **4** | **Create or Provide Configuration (ConfigMap / CR)** <br>• A ConfigMap may list driver parameters (endpoints, managed arrays, port groups, transport protocol, etc.) <br>• In the driver’s CR (e.g. `CSIPowerMax`), define parameters: <br>  • `X_CSI_POWERMAX_ENDPOINT` <br>  • `X_CSI_MANAGED_ARRAYS` <br>  • `X_CSI_TRANSPORT_PROTOCOL` (FC, iSCSI, NVMeTCP, etc.) <br>  • Port groups if using iSCSI <br>  • TLS/revproxy parameters <br>  • Volume prefix, monitor interval, etc. <br>• Ensure mount propagation is allowed |
...
| **7** | **StorageClass Creation** <br>Dell’s operator/CR may auto-create StorageClass resources. <br>Edit as needed (reclaimPolicy, volumeBindingMode, allowVolumeExpansion). |
...
| **9** | **Testing & Validation** <br>• Check `oc get pvc`, `oc describe pvc` <br>• Inside pod, verify mount and read/write <br>• Test expand & deletion <br>• Monitor logs & metrics <br>• Test node failures & reattachment |
| **10** | **Ongoing Operations** <br>• Adjust logging via ConfigMap <br>• Monitor health (if enabled) <br>• Follow Dell’s documented upgrade path for updates |
